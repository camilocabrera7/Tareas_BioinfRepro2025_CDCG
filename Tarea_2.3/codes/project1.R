...
setwd("~/Desktop/Doctorado/Ramos/Semestre 2-2025/Bioinformática/Archivos sesión 6/popgen_shared")
...
#### Load f2_blocks
f2_blocks_p1 <- f2_from_precomp(outdir)
...
f3_results_p1 <- f3(f2_blocks_p1, pop1="Mbuti.DG", pop2=target1, pop3=source1)


#### f4 tests: asymmetry checks. Are target populations closer to any of the potential sources?

f4_results_p1 <- f4(f2_blocks_p1, pop1="GBR.DG", pop2=c("Kazakhstan_Berel_IA.AG", "Basque.DG"), pop3=source1, pop4="Mbuti.DG")


#### qpWave: test rank (how many ancestry streams are needed)
wave_p1 <- qpwave(f2_blocks_p1,
                left = c(target1, source1),
                right = outgroup1)
wave_p1

#### qpAdm: 2-way and 3-way mixture models
admix_2way_p1 <- qpadm(f2_blocks_p1, left= c(target1, source1[1:2]), right = outgroup1, target=target1[1])
admix_3way_p1 <- qpadm(f2_blocks_p1, left = c(target1, source1), right = outgroup1, target=target1[1])
view(admix_2way_p1$weights)
view(admix_3way_p1$weights)
